...
generating a Harbor task and running the trial machinery. Reading Harbor 0.17.1
showed that ``Trial`` owns task download, Docker environment construction,
artifact collection, verifier execution, and result persistence; Wind Tunnel
needs none of Harbor's verifier/reward stack because scenarios do their own
scoring. ``Terminus2`` itself only requires an environment object with a tmux
session surface and an ``AgentContext`` sink, so the direct seam has fewer
moving parts and no ``harbor run`` subprocess.
...
class _TerminusHandle:
    def __init__(
        ...
        *,
        ...
        self._current_environment: _LocalWorkspaceEnvironment | None = None
    ...
    def send(self, messages: list[Message], session_id: str) -> Response:
        ...
        environment = _LocalWorkspaceEnvironment(
