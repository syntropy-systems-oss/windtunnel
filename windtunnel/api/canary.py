...
def run_reset_canary(
    ...
    *,
    ...
    Provisions a handle, seeds a random nonce into session A, calls
    `reset_state()`, then drives probe turns in a *different* session B
    and scans every reply for the nonce. Any hit proves a leak. If
    `state_probe` is given, its post-reset snapshot is scanned too —
    catching the class of leak (e.g. a search index) that transcript
    probing alone misses.
    ...
    proof of isolation — see the module docstring.
